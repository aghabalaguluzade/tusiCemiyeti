...
@section('title', 'Təbiəti')
@section('content')
     <div class="content">
                    <!-- Elements -->
                    <div class="block block-rounded">
                        <div class="block-header block-header-default">
                            <h3 class="block-title">Təbiəti</h3>
                        </div>
                        <div class="block-content">
                        @include('settings.errors')
                            <form action="{{ route('natures.update',1) }}" method="POST" enctype="multipart/form-data">
                              @csrf
                              @method('put')
                                <div class="row push">
                                    <div class="col-lg-12 col-xl-12 overflow-hidden">
                                             @if($nature)
                                                  <img src="{{ asset($nature->img) }}" alt="{{ $nature->img }}" style="width:250px; height:250px" />
                                             @endif
                                        ...
                                        <div class="form-group">
                                            ...
                                            @if($nature)
                                             <textarea class="form-control" id="editor" name="description" rows="4" placeholder="Açıqlama daxil et...">{!! $nature->description !!}</textarea>
                                            @else
