...
@section('title', 'Tusi Klubu')
@section('content')
     <div class="content">
                    <!-- Elements -->
                    <div class="block block-rounded">
                        <div class="block-header block-header-default">
                            <h3 class="block-title">Tusi Klubu</h3>
                        </div>
                        <div class="block-content">
                        @include('settings.errors')
                            <form action="{{ route('tusi.update',1) }}" method="POST" enctype="multipart/form-data">
                              @csrf
                              @method('put')
                                <div class="row push">
                                    <div class="col-lg-12 col-xl-12 overflow-hidden">
                                             @if($view)
                                                  <img src="{{ asset($view->img) }}" alt="{{ $view->img }}" style="width:250px; height:250px" />
                                             @endif
                                        ...
                                        <div class="form-group">
                                            ...
                                            @if($view)
                                             <textarea class="form-control" id="editor" name="description" rows="4" placeholder="Açıqlama daxil et...">{!! $view->description !!}</textarea>
                                            @else
