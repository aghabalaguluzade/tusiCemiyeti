...
@section('title', 'Turizmi')
@section('content')
     <div class="content">
                    <!-- Elements -->
                    <div class="block block-rounded">
                        <div class="block-header block-header-default">
                            <h3 class="block-title">Turizmi</h3>
                        </div>
                        <div class="block-content">
                        @include('settings.errors')
                            <form action="{{ route('tourisms.update',1) }}" method="POST" enctype="multipart/form-data">
                              @csrf
                              @method('put')
                                <div class="row push">
                                    <div class="col-lg-12 col-xl-12 overflow-hidden">
                                             @if($tourism)
                                                  <img src="{{ asset($tourism->img) }}" alt="{{ $tourism->img }}" style="width:250px; height:250px" />
                                             @endif
                                        ...
                                        <div class="form-group">
                                            ...
                                            @if($tourism)
                                             <textarea class="form-control" id="editor" name="description" rows="4" placeholder="Açıqlama daxil et...">{!! $tourism->description !!}</textarea>
                                            @else
