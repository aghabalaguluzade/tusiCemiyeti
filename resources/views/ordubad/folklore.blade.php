...
@section('title', 'Folkloru')
@section('content')
     <div class="content">
                    <!-- Elements -->
                    <div class="block block-rounded">
                        <div class="block-header block-header-default">
                            <h3 class="block-title">Folkloru</h3>
                        </div>
                        <div class="block-content">
                        @include('settings.errors')
                            <form action="{{ route('folklores.update',1) }}" method="POST" enctype="multipart/form-data">
                              @csrf
                              @method('put')
                                <div class="row push">
                                    <div class="col-lg-12 col-xl-12 overflow-hidden">
                                             @if($folklore)
                                                  <img src="{{ asset($folklore->img) }}" alt="{{ $folklore->img }}" style="width:250px; height:250px" />
                                             @endif
                                        ...
                                        <div class="form-group">
                                            ...
                                            @if($folklore)
                                             <textarea class="form-control" id="editor" name="description" rows="4" placeholder="Açıqlama daxil et...">{!! $folklore->description !!}</textarea>
                                            @else
